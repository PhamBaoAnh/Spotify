        ...
        create table `streamify_prod`.`dim_users`
...
static_users AS (
    SELECT
        lower(hex(MD5(toString(coalesce(cast(userId as String), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(level as String), '_dbt_utils_surrogate_key_null_') )))) AS userKey,
        CAST(userId AS Int64) AS userId,
        firstName,
        lastName,
        gender,
...
-- 6️⃣ Kết hợp tất cả
SELECT
    lower(hex(MD5(toString(coalesce(cast(userId as String), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(rowActivationDate as String), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(level as String), '_dbt_utils_surrogate_key_null_') )))) AS userKey,
    userId, firstName, lastName, gender, registration, level,
